...
There are a few minor differences in the mathematical details of the model,
which I had to make in order for the chains to mix properly. I have clearly
commented in this example the places where our models are different.
...
class GP1:
    ...
    def get_data(self):
        ...
        return dict(x=xsd, y=ysd, L=1.5 * max(xsd), M=10,)
...
class GP2:
    ...
    def get_data(self):
        ...
        return dict(x=xsd, y=ysd, w0=w0, J=20, L=1.5 * max(xsd), M=10,)
    ...
    def make_figure(self, samples):
        ...
        y = 100 * data["births_relative"]
        y_by_day_of_year = (
            100 * data.groupby("day_of_year2")["births_relative"].mean()
        )
        μ = 100 * self.y_scaler.inverse_transform(samples["μ"]).mean(axis=0)
...
class GP3:
    ...
    def model(self, x, day_of_week, w0, J, L, M, L3, M3, y=None):
        ...
        ρ3 = sample(
            "ρ3", dist.LogNormal(1.0, 1.0)
        )  # prior: very long-term effect
    ...
    def make_figure(self, samples):
        ...
        axes[0, 0].plot(dates, _agg(μ, scaler=self.y_scaler), color="r", lw=0, label="Total", marker=".", alpha=0.5)
        ...
        axes[1, 0].plot(year_days, _agg(f2[:, :366], scaler=self.y_scaler), color="r", lw=2, label="Year seasonality")
